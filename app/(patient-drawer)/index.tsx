...
import {
  ...
  FlatList,
  Dimensions,
  AppState, // Added for mobile sync
  ActivityIndicator,
} from "react-native";
...
const UserDashboard = () => {
  ...
  // --- STABLE FETCH FUNCTION (For Badges/Updates) ---
  ...
  useEffect(() => {
    ...
    // Refresh when user returns to app from background
    const appStateSub = AppState.addEventListener("change", (nextState) => {
      if (nextState === "active") fetchDashboardData();
    });
...
const styles = StyleSheet.create({
  ...
  // BADGE STYLES
  tabBadge: { position: 'absolute', top: -4, right: -4, backgroundColor: '#EF4444', minWidth: 18, height: 18, borderRadius: 9, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#FFF' },
