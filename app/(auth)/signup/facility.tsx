...
const FacilitySignup = () => {
  ...
  const [customType, setCustomType] = useState(""); // For 'Other' selection
  ...
  // Full list of facility categories including Optics and Others
  const facilityTypes = [
    "hospital", 
    "clinic", 
    "pharmacy", 
  ...
  const handleSignup = async () => {
    // Determine the actual type to save
    const selectedType = facilityType === 'other' ? customType : facilityType;

    if (!facilityName || !selectedType || !email || !password || !registrationNumber || !certificate) {
    ...
    try {
      ...
      // 2. Use signupBase with finalized facility_type
    ...
    <LinearGradient colors={["#0F172A", "#0B3C5D", "#0EA5E9"]} style={{ flex: 1 }}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          ...
          {/* Facility Type Selection Section */}
          <Text style={styles.label}>Select Facility Type</Text>
          <View style={styles.typeContainer}>
            {facilityTypes.map((type) => (
              <TouchableOpacity 
                key={type} 
                style={[styles.typeBadge, facilityType === type && styles.activeBadge]}
                onPress={() => {
                  setFacilityType(type);
                  if (type !== 'other') setCustomType(""); // Reset custom input if switching back
                }}
              >
                <Text style={[styles.typeText, facilityType === type && styles.activeTypeText]}>
                  {type === 'mental_health' 
                    ? 'Mental Health' 
          ...
          {/* Show custom input if 'Other' is picked */}
          {facilityType === 'other' && (
            <View style={styles.inputWrapper}>
              <FontAwesome5 name="pen" size={16} color="#94A3B8" style={styles.icon} />
              <TextInput
                placeholder="Specify type (e.g. Optics, Fitness)"
                placeholderTextColor="#94A3B8"
                style={styles.input}
                value={customType}
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 16, color: "#BAE6FD", marginBottom: 20 },
  label: { color: "#BAE6FD", fontSize: 14, fontWeight: "600", marginBottom: 10, marginLeft: 4 },
  ...
  activeBadge: { backgroundColor: "#E0F2FE", borderColor: "#E0F2FE" },
  typeText: { color: "#BAE6FD", fontSize: 13, fontWeight: "600" },
  activeTypeText: { color: "#0F172A" },
  inputWrapper: {
    flexDirection: "row",
    alignItems: "center",
