...
const FreelancerSignup = () => {
  ...
  // Updated Specializations for iCare Freelancers
  const specOptions = [
    "Medical Doctor",
    "Physician",
    "Pediatrician",
  ...
  const handleSignup = async () => {
    const finalSpec = specialization === "Other" ? customSpecialization : specialization;

    if (!fullName || !email || !password || !certificate || !finalSpec) {
        ...
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.specContainer}>
            {specOptions.map((opt) => (
              <TouchableOpacity 
                key={opt} 
                style={[styles.specBadge, specialization === opt && styles.activeBadge]}
                onPress={() => {
                    setSpecialization(opt);
                    if (opt !== "Other") setCustomSpecialization("");
                }}
              >
                <Text style={[styles.specText, specialization === opt && styles.activeTypeText]}>
                  {opt}
                </Text>
              </TouchableOpacity>
            ))}
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 16, color: "#BAE6FD", marginBottom: 24 },
  label: { color: "#BAE6FD", fontSize: 14, fontWeight: "600", marginBottom: 10, marginLeft: 4 },
  specContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 20 },
  specBadge: { 
  ...
  specText: { color: "#BAE6FD", fontSize: 11, fontWeight: "600" },
